...
def _draw_top_banner(c: canvas.Canvas):
    """Top-left diagonal banner + SPETS logo + address. Drawn on every page."""
    width, height = A4
    c.saveState()
...
def _draw_bottom_banner(c: canvas.Canvas):
    """Bottom-right diagonal banner + phone/email. Only on LAST page."""
    width, height = A4
    c.saveState()
