...
async def gdpr_response(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # =================================================================
    # STEP 1: Create order in KeyCRM FIRST → get real order_id
    # This way the quote number in chat/PDF always matches KeyCRM.
    # =================================================================
    keycrm_url = None
    quote_number = None
    try:
        keycrm_data = {
            ...
            "quote_number": "pending",  # placeholder, real id comes back
            "lang": lang,
            "all_packages": {
        ...
        if crm_result.get("ok"):
            ...
            quote_number = str(crm_result.get("order_id"))  # real id from KeyCRM
            log.info(f"KeyCRM push OK: order_id={quote_number}")
        else:
