...
def generate_quote_pdf(quote_data: dict) -> bytes:
    ...
    # Append company presentation pages after the invoice (same file)
    pdf_bytes = _append_presentation(pdf_bytes)

    return pdf_bytes


def _append_presentation(invoice_bytes: bytes) -> bytes:
    """Append the company presentation PDF after the invoice, if available."""
    pres_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             "presentation.pdf")
    if not os.path.exists(pres_path):
        log.info("presentation.pdf not found - sending invoice only")
        return invoice_bytes
    try:
        from pypdf import PdfReader, PdfWriter
        writer = PdfWriter()
        # invoice pages first
        inv_reader = PdfReader(io.BytesIO(invoice_bytes))
        for page in inv_reader.pages:
            writer.add_page(page)
        # then presentation pages
        pres_reader = PdfReader(pres_path)
        ...
        merged = out.getvalue()
        out.close()
        log.info(f"Presentation appended ({len(pres_reader.pages)} pages)")
        return merged
    except Exception as e:
        log.warning(f"Could not append presentation: {e}")
        return invoice_bytes
