...
try:
    from email_sender import send_3_packages_email
except ImportError:
    send_3_packages_email = None
    logging.getLogger(__name__).warning(
        "send_3_packages_email not available — customer emails disabled"
    )
...
from quote_generator_ajax import generate_ajax_quote_pdf
...
(LANGUAGE, NAME, PHONE, EMAIL, ADDRESS,
 ...
 BIG_PROJECT_CONFIRM, MESSAGE_MANAGER, AFTER_QUOTE,
 # --- Ajax alarm flow ---
 AJAX_MODE, AJAX_KIT, AJAX_HUB, AJAX_MOTION, AJAX_MOTIONCAM,
 AJAX_DOOR, AJAX_LEAK, AJAX_SMOKE, AJAX_SIREN, AJAX_KEYPAD,
 AJAX_KEYFOB, AJAX_REVIEW) = range(26)

# Hand the Ajax flow module its state ids
ajax_flow.init_states({
    "AJAX_MODE": AJAX_MODE, "AJAX_KIT": AJAX_KIT, "AJAX_HUB": AJAX_HUB,
    "AJAX_MOTION": AJAX_MOTION, "AJAX_MOTIONCAM": AJAX_MOTIONCAM,
    "AJAX_DOOR": AJAX_DOOR, "AJAX_LEAK": AJAX_LEAK, "AJAX_SMOKE": AJAX_SMOKE,
    "AJAX_SIREN": AJAX_SIREN, "AJAX_KEYPAD": AJAX_KEYPAD,
    "AJAX_KEYFOB": AJAX_KEYFOB, "AJAX_REVIEW": AJAX_REVIEW,
})
...
async def handle_service(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if choice == "cctv":
        ...
        # Ajax wireless alarm calculator
        return await ajax_flow.enter_ajax(update, context, t, get_lang)
...
async def gdpr_response(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if customer_email and send_3_packages_email is not None:
        try:
...
# =====================================================================
# =====================================================================
# AJAX FLOW — wrapper handlers (pass t + get_lang into ajax_flow module)
# =====================================================================
async def aj_mode(update, context):     return await ajax_flow.handle_mode(update, context, t, get_lang)
async def aj_kit(update, context):      return await ajax_flow.handle_kit(update, context, t, get_lang)
async def aj_hub(update, context):      return await ajax_flow.handle_hub(update, context, t, get_lang)
async def aj_motion(update, context):   return await ajax_flow.handle_motion(update, context, t, get_lang)
async def aj_motioncam(update, context):return await ajax_flow.handle_motioncam(update, context, t, get_lang)
async def aj_door(update, context):     return await ajax_flow.handle_door(update, context, t, get_lang)
async def aj_leak(update, context):     return await ajax_flow.handle_leak(update, context, t, get_lang)
async def aj_smoke(update, context):    return await ajax_flow.handle_smoke(update, context, t, get_lang)
async def aj_siren(update, context):    return await ajax_flow.handle_siren(update, context, t, get_lang)
async def aj_keypad(update, context):   return await ajax_flow.handle_keypad(update, context, t, get_lang)


async def aj_keyfob(update, context):
    res = await ajax_flow.handle_keyfob(update, context, t, get_lang)
    return ConversationHandler.END if res == -1 else res


async def aj_review(update, context):
    """Final confirm → generate Ajax quote PDF, send in Telegram + email."""
    lang = get_lang(context)
    query = update.callback_query
    await query.answer()
    action = query.data.split(":")[1]  # kit / custom / cancel
    ...
    if action == "kit":
        quote = build_kit_quote(data.get("ajax_kit_tier", "balance"))
        crm_label = f"Ajax Kit — {quote['tier']}"
    else:
        quote = build_custom_quote(data.get("ajax_sel", {}))
        crm_label = "Ajax Custom System"
    ...
    try:
        ...
        log.exception("Ajax PDF generation failed")
    ...
    try:
        await context.bot.send_document(
            ...
            caption=t("ajax_quote_caption", lang) if t("ajax_quote_caption", lang) != f"[missing: ajax_quote_caption]"
            else "🛡️ Your Ajax wireless alarm quote",
        )
    except Exception:
        log.exception("Failed to send Ajax PDF in Telegram")

    # Email to customer (single PDF)
    if customer["email"] and ajax_flow is not None:
        try:
            from email_sender import send_single_quote_email
            send_single_quote_email(
                to_email=customer["email"],
                customer_name=customer["name"],
                quote_number=quote_number,
                pdf_bytes=pdf,
                subject_label="Ajax Wireless Alarm",
                lang=lang,
            )
        except ImportError:
            log.warning("send_single_quote_email not available — Ajax email skipped")
        except Exception:
            log.exception("Ajax email send failed")
    ...
    if ADMIN_CHAT_ID:
        try:
            send_admin_notification(
                ...
                object_type=crm_label,
                camera_count=0,
            )
        except Exception:
            log.exception("Admin notify (Ajax) failed")
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            # --- Ajax alarm flow ---
            AJAX_MODE:      [CallbackQueryHandler(aj_mode, pattern="^ajmode:")],
            AJAX_KIT:       [CallbackQueryHandler(aj_kit, pattern="^ajkit:")],
            AJAX_HUB:       [CallbackQueryHandler(aj_hub, pattern="^ajhub:")],
            AJAX_MOTION:    [CallbackQueryHandler(aj_motion, pattern="^ajmot:")],
            AJAX_MOTIONCAM: [CallbackQueryHandler(aj_motioncam, pattern="^ajmcam:")],
            AJAX_DOOR:      [CallbackQueryHandler(aj_door, pattern="^ajdoor:")],
            AJAX_LEAK:      [CallbackQueryHandler(aj_leak, pattern="^ajleak:")],
            AJAX_SMOKE:     [CallbackQueryHandler(aj_smoke, pattern="^ajsmoke:")],
            AJAX_SIREN:     [CallbackQueryHandler(aj_siren, pattern="^ajsir:")],
            AJAX_KEYPAD:    [CallbackQueryHandler(aj_keypad, pattern="^ajkp:")],
            AJAX_KEYFOB:    [CallbackQueryHandler(aj_keyfob, pattern="^ajkf:")],
            AJAX_REVIEW:    [CallbackQueryHandler(aj_review, pattern="^ajok:")],
        },
